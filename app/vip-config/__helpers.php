 ...
 *
 * Something that `add_query_arg()` does, but can be called before WP is loaded.
 *
 * @return string
 */
...
/**
 * Takes a domain or a full URL as input, ensures it is a full absolute URL (prepending the HTTP
 * schema if necessary).
 *
 * Based on the additional parameters, append the current URL path and/or query string to the
 * obtained URL.
 *
 * @param string $base
...
/**
 * Merge given data as query variables to the given URL.
 *
 * Something that `add_query_arg()` does, but can be called before WP is loaded.
 *
 * @param string $url
...
function mergeQueryVars(string $url, array $vars): string
{
    ...
    foreach ($vars as $key => $value) {
        ...
        // Using `null` as value is used to remove a query var.
        // This way it is possible to generically preserve query vars, excluding some of them.
        // To use literal `"null"` in query vars be sure to provide that as string.
        if ($value === null) {
            unset($queryVars[$key]);
            continue;
        }
    ...
    if ($queryVars !== []) {
        $baseUrl = "{$baseUrl}?" . http_build_query($queryVars, encoding_type: PHP_QUERY_RFC3986);
    }
 ...
 *
 * Excluding production, a random string returned if the file is not found. The reason for that is
 * this ID is often used to invalidate cache, and so it makes sense to fall back to an
 * always-changing random value when not in production.
 *
 * @return non-falsy-string|null
 */
